		...
		z = function(i) {
			...
			if (a[0]) {
				...
				return "none" === w.css("display") && w.show(), k && t.close(k.attr("times")), 1 !== s.find("li").length || o[0] || (d || w.css("width", 800), s.css({
					height: w.height()
				}).show(), c.css("margin-left", "200px")), o[0] || (s.append(n(v).render(l)), c.append(n(f).render(l)), I(i), E()), O(s.find(".layim-chatlist-" + i.type + i.id)), o[0] || P(), $(i), Q(), C
			}
			l.first = !0;
			var u = C = t.open({
				type: 1,
		...
		ti = {
			...
			image: function(i) {
				var a = i.data("type") || "images",
					ac = i.data("accept") || "images",
					am = i.data("acceptMime") || "image/*",
					e = {
						images: "uploadImage",
						file: "uploadFile"
					},
