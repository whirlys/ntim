...
<body>
	<div class="layui-layout layui-layout-admin kit-layout-admin">
		<div class="layui-header">
			<div class="layui-logo">NTIM 学生端</div>
			...
			<div class="layui-side-scroll">
				...
				<ul class="layui-nav layui-nav-tree" lay-filter="kitNavbar"
					...
					<li class="layui-nav-item"><a href="javascript:;" kit-target
						data-options="{url:'${ctx }/student/toNotices',icon:'&#xe658;',title:'我的通知',id:'1'}"><i
							class="layui-icon">&#xe658;</i>我的通知<span id="notRead"
							class="layui-badge">0</span> </a></li>
					<li class="layui-nav-item"><a href="javascript:;" kit-target
						data-options="{url:'${ctx }/question',icon:'&#xe658;',title:'问答专区',id:'2'}"><i
							class="layui-icon">&#xe658;</i><span> 问答专区</span></a></li>

					<li class="layui-nav-item"><a href="javascript:;" kit-target
						data-options="{url:'${ctx }/student/info',icon:'&#xe658;',title:'个人信息管理',id:'3'}"><i
							class="layui-icon">&#xe658;</i><span> 个人信息管理</span></a></li>
	...
	<script>
		...
		layui.config({
			base : '${ctx}/js/student/'
		}).use([ 'layer', 'jquery', 'layim', 'util', 'app', 'message' ], function() {
			var layer = layui.layer, $ = layui.jquery, layim = layui.layim, util = layui.util;
			...
			layim.config({
				...
				init : {//获取主面板列表信息，下文会做进一步介绍
					url : '${ctx}/student/layim/init',//接口地址（返回的数据格式见下文）
				...
				//获取群员接口（返回的数据格式见下文）
				members : {
					url : '${ctx}/student/layim/members' //接口地址（返回的数据格式见下文）
					,
					type : 'get' //默认get，一般可不填
					,
					data : {}
			...
			var getNotReadNum = function() {
				var span = $("#notRead");

				$.get("${ctx}/student/notReadNum", function(data) {
					if (data.code == "100") {
						span.text(data.map.notReadNum)
					}
				});
			}
			...
			var initEventHandle = function() {
				...
				socket.onmessage = function(event) {
					if (event.data instanceof ArrayBuffer) {
						...
						} else if (msg.getCmd() == 7) { // 推送的消息
							var body = JSON.parse(msgCon.getContent());
							var title = msgCon.getTitle() + " : " + body.title;
							var options = {
								dir : "rtl",
								lang : "zh-CN",
								body : body.sampleContent,
								icon : "${ctx}/images/notify.jpg",
							}
