				...
				<c:forEach items="${studentGroups }" var="group">
					<tr>
						<td><input type="checkbox" class="groupbox"
							title="${group.groupname }" lay-skin="primary" lay-filter="group"
							id="${group.id }"></td>
						<td><c:forEach items="${group.list }" var="item">
								<input type="checkbox" title="${item.username }" name="student"
									lay-filter="student" id="${item.id }" gid="${group.id }">
							</c:forEach></td>
					</tr>
				</c:forEach>
			</tbody>
	...
	<script>
		layui.use([ 'layedit', 'layer', 'jquery', 'form' ], function() {
			...
			form.on('submit(*)', function(data) {
				var ids = [];
				$("input[type='checkbox'][name='student']").each(function() {
