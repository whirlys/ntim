...
public class ExcelUtil {
		...
		if (workbook != null) {
			for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
				...
				for (int rowNum = firstRowNum; rowNum <= lastRowNum; rowNum++) {
					...
					// 如果当前行第一个cell为空字符串，则当行作废
					if (row.getPhysicalNumberOfCells() != 0 && getCellValue(row.getCell(0)).equals("")) {
						continue;
					}
					// 获得当前行的开始列
	...
	public static Map<String, Integer> ExcelHeaderMap() {
		...
		headerMap.put("专业", -1);
		headerMap.put("班级号", -1);
		headerMap.put("学生类型", -1);
		headerMap.put("性别", -1);
		headerMap.put("生日", -1);
		headerMap.put("长号", -1);
		headerMap.put("短号", -1);
		headerMap.put("职务", -1);
		headerMap.put("家庭电话", -1);
		headerMap.put("邮箱", -1);
		headerMap.put("QQ", -1);
		headerMap.put("身份证号", -1);
		headerMap.put("家庭住址", -1);
		return headerMap;
	}
}